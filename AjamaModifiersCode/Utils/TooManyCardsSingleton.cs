...
public class TooManyCardsSingleton() : CustomSingletonModel(HookType.Combat), IMaxHandSizeModifier
{
    ...
    /// If Too Many Friendships is enabled, remove matching Power cards when another player plays one.
    public override async Task BeforeCardPlayed(CardPlay cardPlay)
    {
        if (!Config.TooManyCardsPowerOfFriendship) return;
...
{
    /// If Too Many Friendships is enabled, mark every Power that can be added to the deck with a unique ID. This lets the other-player copies generated by Too Many Cards be linked and get removed from combat when one of them is played.
    public override bool TryModifyCardBeingAddedToDeck(CardModel card, out CardModel? newCard)
    {
        newCard = null;
        
        if (!Config.TooManyCardsPowerOfFriendship) return false;
